...
export class PythonASTConverter {
    ...
    if (bodyNode) {
      for (const member of bodyNode.children) {
        if (member.type === "function_definition") {
          ...
          const funcDef = member.children.find(
  ...
  private extractClassVariable(
    ...
    const isArray = varType
      ? varType.endsWith("[]") ||
        varType.startsWith("Array<") ||
        varType === "Array"
      : false;
  ...
  private extractImport(node: SyntaxNode): ImportInfo[] {
    ...
    if (node.type === "import_statement") {
      ...
      if (importList) {
        ...
        const aliasNode = node.children.find(
          (c) => c.type === "aliased_import",
        );
      ...
      if (importList) {
        for (const importItem of importList.children) {
          if (
            importItem.type === "dotted_name" ||
            importItem.type === "identifier"
          ) {
  ...
  private extractTypeString(node: SyntaxNode): string | undefined {
    if (node.type === "identifier") {
      ...
      const identifierNode = node.children.find((c) => c.type === "identifier");
      const typeParamNode = node.children.find(
   ...
   */
  private extractInstanceVariablesFromInit(
    initNode: SyntaxNode,
  ): PropertyInfo[] {
    const properties: PropertyInfo[] = [];
   ...
   */
  private extractPropertyFromAssignment(
    assignmentNode: SyntaxNode,
  ): PropertyInfo | null {
    const leftNode = assignmentNode.childForFieldName("left");
    ...
    // Check if type is an array
    const isArray = propertyType
      ? propertyType.endsWith("[]") ||
        propertyType.startsWith("Array<") ||
        propertyType === "Array"
      : false;
