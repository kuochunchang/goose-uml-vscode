...
describe('CrossFileAnalyzer', () => {
  ...
  describe('analyzeForward', () => {
    it('should throw error for invalid depth', async () => {
      ...
      await expect(analyzer.analyzeForward('/src/User.ts', 0 as any)).rejects.toThrow(
        'Depth must be between 1 and 3'
      );

      await expect(analyzer.analyzeForward('/src/User.ts', 4 as any)).rejects.toThrow(
        'Depth must be between 1 and 3'
      );
    });

    it('should throw error if file does not exist', async () => {
      await expect(analyzer.analyzeForward('/src/NonExistent.ts', 1)).rejects.toThrow(
    ...
    it('should analyze dependencies at depth 2', async () => {
      ...
      fileProvider.addFile('/src/UserService.ts', TS_FIXTURES.serviceWithDependency);
...
export class C {
  ...
  describe('analyzeBidirectional', () => {
    it('should analyze forward dependencies', async () => {
      ...
      const result = await analyzer.analyzeBidirectional('/src/UserService.ts', 2);
    ...
    it('should handle inheritance relationships', async () => {
      fileProvider.addFile('/src/UserService.ts', TS_FIXTURES.classWithInheritance);
      fileProvider.addFile('/src/BaseService.ts', TS_FIXTURES.baseService);

      const result = await analyzer.analyzeBidirectional('/src/UserService.ts', 2);

      // UserService should have extends BaseService
      const userServiceClass = result.allClasses.find((c) => c.name === 'UserService');
    ...
    it('should handle interface implementation', async () => {
      fileProvider.addFile('/src/ConsoleLogger.ts', TS_FIXTURES.classWithInterface);
      fileProvider.addFile('/src/ILogger.ts', TS_FIXTURES.interface);

      const result = await analyzer.analyzeBidirectional('/src/ConsoleLogger.ts', 2);

      // ConsoleLogger should implement ILogger
      const loggerClass = result.allClasses.find((c) => c.name === 'ConsoleLogger');
  ...
  describe('extractReferencedClasses (indirect test via analyzeBidirectional)', () => {
    it('should extract classes from extends relationship', async () => {
      ...
      const hasBaseService = result.allClasses.some((c) => c.name === 'BaseService');
...
public class User {
    ...
    public String getName() {
      ...
      await expect(analyzer.analyzeForward('/src/User.java', 1)).rejects.toThrow(
...
class User:
  ...
  describe('error handling', () => {
    it('should handle malformed TypeScript gracefully', async () => {
      fileProvider.addFile('/src/Invalid.ts', 'this is not valid typescript {{{');

      // Should throw parsing error
      await expect(analyzer.analyzeForward('/src/Invalid.ts', 1)).rejects.toThrow();
    });
