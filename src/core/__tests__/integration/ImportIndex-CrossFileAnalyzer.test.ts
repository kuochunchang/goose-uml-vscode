...
export class UserService {
  private user: User;
      ...
      const analyzerWithIndex = new CrossFileAnalyzer(
        fileProvider,
        importIndex,
      );
      const startWithIndex = Date.now();
      const resultsWithIndex = await analyzerWithIndex.analyzeForward(
        "/src/services/UserService.ts",
        2,
      );
      ...
      console.log(
        `[Performance] Without ImportIndex: ${durationWithoutIndex}ms`,
      );
      console.log(
...
export class UserRepository {
  private users: User[];
    ...
    it("should produce identical analysis results", async () => {
      ...
      const analyzerWithoutIndex = new CrossFileAnalyzer(fileProvider);
      const resultsWithoutIndex =
        await analyzerWithoutIndex.analyzeBidirectional(
          "/src/controllers/UserController.ts",
          3,
        );

      // Compare results
      expect(resultsWithIndex.allClasses.length).toBe(
        resultsWithoutIndex.allClasses.length,
      );
      expect(resultsWithIndex.relationships.length).toBe(
        resultsWithoutIndex.relationships.length,
      );
      expect(resultsWithIndex.stats.totalFiles).toBe(
        resultsWithoutIndex.stats.totalFiles,
      );

      // Verify all class names match
      const classNamesWithIndex = resultsWithIndex.allClasses
        .map((c) => c.name)
        .sort();
      const classNamesWithoutIndex = resultsWithoutIndex.allClasses
        .map((c) => c.name)
        .sort();
...
export class App {
  ...
  describe("Edge Cases", () => {
    it("should handle same class name in different directories", async () => {
      fileProvider.addFile(
        "/src/models/User.ts",
        'export class User { type = "model"; }',
      );
      fileProvider.addFile(
        "/src/entities/User.ts",
        'export class User { type = "entity"; }',
      );
      fileProvider.addFile(
        "/src/App.ts",
        `
  ...
  describe("Performance Benchmarking", () => {
    it("should scale well with large number of files", async () => {
      ...
      const imports = Array.from(
        { length: 10 },
        (_, i) => `import { Class${i} } from './Class${i}';`,
      ).join("\n");
      const classRefs = Array.from(
        { length: 10 },
        (_, i) => `private class${i}: Class${i};`,
      ).join("\n  ");
...
export class App {
  ${classRefs}
      ...
      console.log(
        `[Benchmark] Index build time for 100 files: ${indexDuration}ms`,
      );
      ...
      console.log(
        `[Benchmark] Analysis time with ImportIndex: ${analysisDuration}ms`,
      );
